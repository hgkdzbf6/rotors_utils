...
#include "rotors_utils/joy_pose_follower.h"

#include <mav_msgs/default_topics.h>
#include <geometry_msgs/PoseStamped.h>

JoyPoseFollower::JoyPoseFollower():yaw_(0) {
  ros::NodeHandle nh;
  ros::NodeHandle pnh("~");
  ...
  timer_=nh_.createTimer(ros::Duration(0.1), &JoyPoseFollower::TimerCallback,this);
  ...
  joy_sub_ = nh_.subscribe("joy", 10, &JoyPoseFollower::JoyCallback, this);
}

void JoyPoseFollower::TimerCallback(const ros::TimerEvent& e){
...
void JoyPoseFollower::JoyCallback(const sensor_msgs::JoyConstPtr& joy) {
  current_joy_ = *joy;
}

double JoyPoseFollower::GetAxis(const Axis &axis){
...
bool JoyPoseFollower::GetButton( const Button &button)
{
