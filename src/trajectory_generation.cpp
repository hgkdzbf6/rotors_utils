...
TrajectoryGeneration::TrajectoryGeneration(){
  ...
  // 第三位是从哪个坐标系
  // 符号最好统一下吧
  T_L2_F2_sub_=nh_.subscribe("relative_position01",5,&TrajectoryGeneration::T_L2_F2Callback,this);
  // leader在leader SVO坐标系下面的坐标
  T_LS_L2_sub_=nh_.subscribe("svo/fusion_pose",5,&TrajectoryGeneration::T_LS_L2Callback,this);
  // follower在follower SVO坐标系下面的坐标
  T_FS_F2_sub_=nh_.subscribe("/hummingbird1/svo/fusion_pose",5,&TrajectoryGeneration::T_FS_F2Callback,this);
  // 在leader坐标系下，follower的期望位置
  // TODO: 这块逻辑应该改改，应该是leader给的位置。日后再改吧。
  T_L2_dF2_sub_=nh_.subscribe("/hummingbird1/leader_desired_position",5,&TrajectoryGeneration::T_L2_dF2Callback,this);
  // 最后输出的位置。应该给控制器的。
  follower_pub_=nh_.advertise<geometry_msgs::PoseStamped>("target_pose",5);
...
void TrajectoryGeneration::TimerCallback(const ros::TimerEvent & e){
  ...
  // 这边广播出来，没啥问题。
  // 应该是上面的定时器的问题。
  // S_F_dF_ = S_L_F_ * S_L2_L_ * S_L2_dF_.inverse();  // S_LW_FW_ = S_LS_LW_.inverse() * S_LS_L2_ * S_L2_F2_ * S_FS_F2_.inverse() * S_FS_FW_;
  ...
  // ROS_INFO_STREAM("S_L_F_:"<<std::endl<<S_L_F_<<std::endl);
  // ROS_INFO_STREAM("S_L2_L_:"<<std::endl<<S_L2_L_<<std::endl);
  // ROS_INFO_STREAM("S_L2_dF_:"<<std::endl<<S_L2_dF_<<std::endl);
  // ROS_INFO_STREAM("S_F_dF_:"<<std::endl<<S_F_dF_<<std::endl);
